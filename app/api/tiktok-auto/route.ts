...
export async function GET() {
  try {
    ...
    console.log(`🚀 Ejecutando scraper para @fruta.za...`);
    ...
    // 🆕 Enriquecer con datos de oEmbed (thumbnails + embed HTML)
    const videosWithMetadata = await Promise.all(
      results.slice(0, 20).map(async (item: any) => {
        const videoId = item.webVideoUrl?.split('/video/')[1] || '';
        
        // Obtener datos de oEmbed para thumbnail y embed
        let oembedData = null;
        try {
        ...
        return {
          ...
          embed_html: oembedData?.html || '', // 🆕 HTML para embed
          stats: {
